...
**Datos**
- Toda tabla nueva lleva `user_id` y su política RLS **en la misma migración**. Sin excepciones.
...
- El webhook de Telegram valida `secret_token` de cabecera y lista blanca de `chat_id`.
...
- Nombres de tablas y columnas en `snake_case` e **inglés**. Textos de la interfaz en **español**.
...
**Fase 0 — Cimientos.** Repositorio recién iniciado. Ver `docs/estado.md`.
